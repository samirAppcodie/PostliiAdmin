...
//       try {
//         const res = await fetch(`https://admin.postlii.com/api/categories`);
...
//           xhr.open('POST', `https://admin.postlii.com/api/videos`);
...
//        style={{
//     background: 'linear-gradient(180deg, #1E1E2F, #4B4F7A)', // new gradient background
...
const handleSubmit = async (e) => {
  ...
  const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
  const useSequential = isSafari || videoFile.length > 10; // Key change: Trigger sequential for large batches or Safari

  try {
    if (useSequential) {
      toast.info(useSequential && !isSafari ? `Using sequential upload for ${videoFile.length} videos to avoid server overload.` : 'Safari detected. Using optimized upload method...');
      ...
      for (let i = 0; i < videoFile.length; i++) {
        ...
        singleFormData.append('thumbnails', thumbnailImages[i]);
        singleFormData.append('title', title);
        singleFormData.append('description', description); // Added: If you add the input, this will include it
        singleFormData.append('categoryId', selectedCategory);
        ...
        xhr.open('POST', `https://admin.postlii.com/api/videos`);
        xhr.responseType = 'json';
        xhr.timeout = 600000; // 10min timeout per file
        xhr.send(singleFormData);
        ...
        try {
          ...
          // Delay between uploads to ease server load (adjust as needed)
          if (i < videoFile.length - 1) {
            await new Promise(resolve => setTimeout(resolve, 1000));
          }
        } catch (error) {
          ...
          toast.error(`Error uploading video ${i + 1}. Continuing with remaining files.`);
          // Optional: Break on error? Or continue as-is.
        }
      }

      toast.success('Upload process completed!');
    } else {
      // Batch upload for small non-Safari batches (≤10 files)
      const formData = new FormData();
      videoFile.forEach((file, index) => {
        formData.append('videos', file);
      });
      ...
      formData.append('title', title);
      formData.append('description', description); // Added: If you add the input
      formData.append('categoryId', selectedCategory);
