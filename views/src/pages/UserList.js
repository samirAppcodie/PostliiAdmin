...
const ConfirmDeleteToast = ({ onConfirm, onCancel }) => (
 ...
 const CustomPagination = ({
  ...
  const handleRowsPerPageChange = (e) => {
    ...
    const newRowsPerPage = value === 'All' ? rowCount : parseInt(value);
    onChangeRowsPerPage(newRowsPerPage, 0); // Reset to page 1
  };
  ...
  // Determine display value for select (show 'All' if rowsPerPage equals total rowCount)
  const displayRowsPerPage = rowsPerPage === rowCount ? 'All' : rowsPerPage;

  // Manual calculation for from/to (fixes "0-0" if props are undefined/0)
  ...
  return (
    <div
      style={{
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'flex-end', // All elements pushed to right-end like requested
        padding: '8px 12px', // Compact like SS
        backgroundColor: '#ffffff', // Light white like SS
        color: '#333333', // Dark text like SS
        fontSize: '14px',
        borderTop: '1px solid #dee2e6', // Subtle like SS
    ...
    >
      {/* Right-grouped: Dropdown + Page Info + Arrows (sequential, clustered right) */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}> {/* 12px gap between groups for balance */}
        {/* Dropdown (now on right side) */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}> {/* Tighter internal gap */}
          ...
          >
            {[15, 25, 50, 100,200, 'All'].map((option) => (
        ...
        {/* Page info (now on right side, next to dropdown - fixed with manual calc) */}
        <div style={{ fontSize: '14px', fontWeight: 500, whiteSpace: 'nowrap' }}>
          {rowCount > 0 
            ? `${calculatedFrom}-${calculatedTo} of ${rowCount}`  // e.g., "1-15 of 162"
            : '0 of 0' // Fallback for no data
          }
        </div>

        {/* Arrows (immediately after page info, on right end) */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}> {/* Tight gap for flush right-end */}
          <button
...
const handleDeleteSelected = () => {
      ...
      onConfirm={async () => {
        try {
          const deletePromises = selectedRows.map(user =>
            axios.delete(`https://admin.postlii.com/api/signup/${user._id}`, {
              headers: { Authorization: `Bearer ${token}` },
            })
          );
          await Promise.all(deletePromises);
          setSignupUsers(prev => prev.filter(user => !selectedRows.includes(user)));
          toast.success('Selected users deleted successfully!');
          setSelectedRows([]);
          setToggleCleared(!toggleCleared); // uncheck all
        } catch (error) {
          toast.error("Bulk delete failed: " + (error.response?.data?.message || error.message));
        }
      }}
      onCancel={() => toast.info('Bulk deletion cancelled')}
    />,
    {
      position: 'top-center',
      autoClose: false,
  ...
  const fetchUsers = async () => {
    ...
    if (!token) return;
    try {
      const response = await axios.get(`https://admin.postlii.com/api/signupall`, {
        headers: {
  ...
  const handleDelete = async (userId) => {
    ...
    toast.info(
      <ConfirmDeleteToast
        onConfirm={async () => {
          try {
            await axios.delete(`https://admin.postlii.com/api/signup/${userId}`, {
              headers: { Authorization: `Bearer ${token}` },
            });
  ...
  const columns = [
    ...
    { name: 'Subscribed', selector: (row) => row.is_subscribed ? 'Subscribed' : 'Trial', sortable: true, grow: 1 },
    {
      name: 'Actions',
      cell: (row) => (
  ...
  >
    Delete Selected ({selectedRows.length})
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '20px' }}>
        <label style={{ color: '#fff', fontSize: '16px' }}>Subscription Status:</label>
        <select
 ...
 <DataTable
  ...
  data={signupUsers.filter((user) => {
    ...
    const matchesSubscription =
      ...
      (subscriptionFilter === 'true' && user.is_subscribed) ||
      (subscriptionFilter === 'false' && !user.is_subscribed);
  ...
  paginationRowsPerPageOptions={[15, 25, 50, 100, 200, 'All']}
  ...
  paginationComponent={(props) => (
    <CustomPagination
      {...props}
      rowCount={signupUsers.length}
      onChangeRowsPerPage={(newPerPage) => props.onChangeRowsPerPage(newPerPage, 0)}
      ...
      {showModal && selectedUser && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p><strong>Is Subscribed:</strong> {selectedUser.is_subscribed ? 'Subscribed' : 'Trial'}</p>
